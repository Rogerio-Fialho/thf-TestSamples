...
@Component({
  selector: 'sample-thf-notification-sales',
  templateUrl: './sample-thf-notification-sales.component.html',
  providers: [ThfNotificationService]
})

export class SampleThfNotificationSalesComponent {
  ...
  readonly columns: Array<ThfTableColumn> = [
    { column: 'productID', label: 'Id' },
    { column: 'productName', label: 'Product'},
    { column: 'quantity', label: 'Quantity' },
    { column: 'price', label: 'Price', type: 'currency', format: 'BRL' },
    { column: 'total', label: 'Total Price', type: 'currency', format: 'BRL' }
  ];
  ...
  readonly productOptions: Array<ThfComboOption> = [
    { value: '001', label: 'T-Shirt Blue' },
  ...
  addCart() {
    ...
    if (this.productOptions && this.quantity > 0) {
      ...
      this.thfNotification.success('Order included successfully!');
  ...
  checkQuantity() {
    if (this.quantity > this.stock ) {
      this.thfNotification.error('Quantity not available in stock');
    } else {
      this.totalValue();
    }
  }
